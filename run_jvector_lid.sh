...
testdir=dev_1s  # you may change it to dev_1s, dev_3s or test
exp=exp/bnfjvector2048_400
mkdir -p $exp
...
if [ $stage -le 3 ]; then
  ...
  sid/train_jvector_extractor.sh --nj 10 --cmd "$cpu_cmd -l mem_free=2G" \
    --num-iters 10 --jvector_dim $civ $exp/full_ubm_${cnum}/final.ubm data/train \
    $exp/extractor_${cnum}_${civ}

  
  # Extract j-vector
  sid/extract_jvectors.sh --cmd "$cpu_cmd -l mem_free=2G," --nj 10 \
...
# Demonstrate what happens if we whiten j-vectors
if [ $stage -le 5 ]; then
  # trials=local/olr/ivector/trials.trl
  # mkdir -p $exp/score/total
