...
if [ $stage -le 0 ]; then
  ...
  $cmd --num-threads $num_threads JOB=1:$nj $dir/log/extract_jvectors.JOB.log \
    scale-post "ark,s,cs:gunzip -c $srcdir/post.final.JOB.gz|" $posterior_scale ark:- \| \
    jvector-extract --verbose=2 --num-threads=$num_threads $srcdir/final.je "$feats" \
      ark,s,cs:- ark,scp,t:$dir/jvector.JOB.ark,$dir/jvector.JOB.scp || exit 1;
fi

if [ $stage -le 1 ]; then
  echo "$0: combining iVectors across jobs"
  for j in $(seq $nj); do cat $dir/jvector.$j.scp; done >$dir/jvector.scp || exit 1;
fi
