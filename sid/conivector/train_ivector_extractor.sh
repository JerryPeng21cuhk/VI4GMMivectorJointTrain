...
while [ $x -lt $num_iters ]; do
  if [ $stage -le $x ]; then
    ...
    for j in $(seq $nj_full); do
      Args[$j]=`echo "ivector-extractor-conv-acc-stats --num-threads=$num_threads  $dir/$x.ie '$feats' 'ark,s,cs:gunzip -c $dir/post.JOB.gz|' -|" | sed s/JOB/$j/g`
    done
    ...
    for g in $(seq $nj); do
      ...
      $cmd $parallel_opts $dir/log/acc.$x.$g.log \
        ivector-extractor-conv-sum-accs --parallel=true "${Args[@]:$start:$num_processes}" \
    ...
    $cmd $sum_accs_opt $dir/log/sum_acc.$x.log \
      ivector-extractor-conv-sum-accs $accs $dir/acc.$x || exit 1;
    ...
    $cmd $dir/log/update.$x.log \
      ivector-extractor-conv-est  $dir/$x.ie $dir/acc.$x $dir/$[$x+1].ie || exit 1;
    rm $dir/acc.$x.*
