...
# This script trains the j-vector extractor.  Note: there are 3 separate levels
...
jvector_dim=400 # dimension of the extracted i-vector
...
if [ $stage -le -2 ]; then
  ...
  $cmd $dir/log/init.log \
    jvector-extractor-init --jvector-dim=$jvector_dim \
     $dir/final.ubm $dir/0.je || exit 1
fi
...
if [ $stage -le -1 ]; then
  ...
  $cmd JOB=1:$nj_full $dir/log/gselect.JOB.log \
    ...
    fgmm-global-gselect-to-post --min-post=$min_post $dir/final.ubm "$feats" \
      ark,s,cs:-  ark:- \| \
    scale-post ark:- $posterior_scale "ark,f:|gzip -c >$dir/post.0.JOB.gz" || exit 1;
  # use ark,f to force flushing write_buffer. jerrypeng
else
...
while [ $x -lt $num_iters ]; do
  if [ $stage -le $x ]; then
    ...
    for j in $(seq $nj_full); do
      Args[$j]=`echo "jvector-extractor-acc-stats --num-threads=$num_threads $dir/$x.je '$feats' 'ark,s,cs:gunzip -c $dir/post.${x}.JOB.gz|' 'ark:|gzip -c > $dir/post.$[$x+1].JOB.gz' -|" | sed s/JOB/$j/g`
    done
    ...
    for g in $(seq $nj); do
      ...
      $cmd $parallel_opts $dir/log/acc.$x.$g.log \
        jvector-extractor-sum-accs --parallel=true "${Args[@]:$start:$num_processes}" \
    ...
    $cmd $sum_accs_opt $dir/log/sum_acc.$x.log \
      jvector-extractor-sum-accs $accs $dir/acc.$x || exit 1;
    ...
    $cmd $dir/log/update.$x.log \
      jvector-extractor-est $dir/$x.je $dir/acc.$x $dir/$[$x+1].je || exit 1;
    rm $dir/acc.$x.*
    $cleanup && rm $dir/acc.$x $dir/$x.je
  fi
  x=$[$x+1]
done
...
for j in $(seq $nj); do
  ln -rfs $dir/post.$num_iters.$j.gz $dir/post.final.$j.gz || echo "failed to create link for final post.$num_iters.$j.gz!" && exit 1;
done

rm -f $dir/final.je
ln -fsr $x.je $dir/final.je
