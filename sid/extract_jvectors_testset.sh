...
if [ $# != 3 ]; then
  echo "Usage: $0 <extractor-dir> <data> <jvector-dir>"
  echo " e.g.: $0 exp/extractor_2048_male data/train_male exp/jvectors_male"
...
for f in $srcdir/final.je $srcdir/final.ubm $data/feats.scp ; do
...
if [ $stage -le 0 ]; then
  echo "$0: extracting jVectors"
  dubm="fgmm-global-to-gmm $srcdir/final.ubm -|"

  $cmd --num-threads $num_threads JOB=1:$nj $dir/log/extract_jvectors.0.JOB.log \
    ...
    jvector-extract --verbose=2 --num-threads=$num_threads $srcdir/final.je "$feats" \
      ark,s,cs:- ark,scp,t:$dir/jvector.0.JOB.ark,$dir/jvector.0.JOB.scp "ark,f:|gzip -c >$dir/jpost.0.JOB.gz" || exit 1;

fi

if [ $stage -le 1 ]; then
   echo "$0: Iteratively extracting jVectors"
   for i in `seq $iter`; do
     echo "Iteration: $i "
     $cmd --num-threads $num_threads JOB=1:$nj $dir/log/extract_jvectors.${i}.JOB.log \
       scale-post "ark,s,cs:gunzip -c $dir/jpost.$[$i-1].JOB.gz|" $posterior_scale ark:- \| \
       jvector-extract --verbose=2 --num-threads=$num_threads $srcdir/final.je "$feats" \
	 ark,s,cs:- ark,scp:$dir/jvector.${i}.JOB.ark,$dir/jvector.${i}.JOB.scp "ark,f:|gzip -c >$dir/jpost.${i}.JOB.gz" || exit 1;
   done
fi

if [ $stage -le 2 ]; then
  echo "$0: combining iVectors across jobs"
  for j in $(seq $nj); do cat $dir/jvector.${iter}.$j.scp; done >$dir/jvector.scp || exit 1;
fi
...
if [ $stage -le 3 ]; then
  ...
  echo "$0: computing mean of jVectors for each speaker and length-normalizing"
  $cmd $dir/log/speaker_mean.log \
    ivector-normalize-length scp:$dir/jvector.scp  ark:- \| \
    ivector-mean ark:$data/spk2utt ark:- ark:- ark,t:$dir/num_utts.ark \| \
    ivector-normalize-length ark:- ark,scp:$dir/spk_jvector.ark,$dir/spk_jvector.scp || exit 1;
fi
