...
if [ $stage -le 4 ]; then
  ...
  lda)
    steps/train_lda_mllt.sh --cmd "$train_cmd" --num-iters 13 \
      ...
      data/$lda_mllt_lang/lang exp/$lda_mllt_lang/tri3b_ali${suffix} exp/$lda_mllt_lang/nnet3${nnet3_affix}/tri4b
    ;;
  pca)
    ...
    steps/online/nnet2/get_pca_transform.sh --cmd "$train_cmd" \
      ...
      exp/$lda_mllt_lang/nnet3${nnet3_affix}/tri4b
    ;;
...
if [ $stage -le 5 ]; then
  ...
  steps/online/nnet2/train_diag_ubm.sh --cmd "$train_cmd" --nj $nj --num-frames 200000 \
    $multi_data_dir $numGaussUBM exp/$lda_mllt_lang/nnet3${nnet3_affix}/tri4b $global_extractor_dir/diag_ubm
fi
